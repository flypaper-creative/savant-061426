...
const anchor = "const isMuted = playSound.getMuted();";
const hookData = `
    ...
    const pool = [
      ...
      { id: 'hull', icon: Sparkles, color: '#10b981', title: 'HULL PLATING', desc: 'Instant emergency repair: Restores 100% of deflector shields immediately.', category: 'SURVIVAL', 
        apply: (setPerks, setShield) => { 
           setShield(9999); // maxShield caps it in AsteroidCanvas anyway! Oh wait, AsteroidCanvas caps on next pick up or hit. Let's just setShield(perks.maxShield)
        }
      }
    ];
